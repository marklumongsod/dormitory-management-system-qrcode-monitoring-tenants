...
$query = "SELECT *, CASE WHEN DATE(date_created) = CURDATE() THEN 1 ELSE 0 END AS is_new_record
          FROM tenants
          ORDER BY is_new_record DESC, tenants_id DESC";
...
<meta charset="utf-8" />
  ...
  <title>DMS Student Boarder List</title>
    <!--     Fonts and icons     -->
...
<body class="g-sidenav-show   bg-gray-100">
  ...
  <aside class="sidenav bg-white navbar navbar-vertical navbar-expand-xs border-2 border-radius-md my-3 fixed-start ms-4 " id="sidenav-main" style="border: solid #046621;">
      ...
      <div class="container-fluid py-1 px-3">
        <div class="collapse navbar-collapse mt-sm-0 mt-2 me-md-0 me-sm-4" id="navbar">
          ...
          <div class="card-body custom-color p-3 shadow">
            <div class="row">
              <div class="col-8">
                <h3 class="text-right mb-0" style="text-transform: uppercase; font-size: 20px; color: #046621; padding: 10px;">Student Boarder List</h3>
              </div>
            </div>
          </div>
          <!--Add Button-->
          <div class="d-flex m-3 justify-content-start"> 
            <a class="button-option me-2" id="GFG" href="insert.php">
            ...
            <!--<a class="button-option me-2" id="GFG" href="updateroomassignment.php">
                <span class="transition"></span>
                <span class="gradient"></span>
                </i><span class="label">Re-assign student room</span>
            </a>-->
            </div>
                 ...
                 <table id="table" class="table bg-white rounded shadow table-striped table-hover" style="width:100% ">
                        <thead>
                            <tr>
                                ...
                                <th scope="col" width="8%">Student No.</th>
                                <th scope="col" width="10%">Student</th>
                                <th scope="col" width="15%">Course</th>
                                <th scope="col" width="3%">Dorm/Room</th>
...
while ($row = mysqli_fetch_array($result)) {
    ...
    $created_date = date('Y-m-d', strtotime($row['date_created']));
...
usort($records, function ($a, $b) {
    if ($a['new_record'] && !$b['new_record']) {
        return -1; 
    } elseif (!$a['new_record'] && $b['new_record']) {
        return 1; 
    } else {
        return 0; 
    }
...
foreach ($records as $row) {
    $new_record = $row['new_record'] ? "<span style='color: red;'>New record!</span>" : "";
    ?>
    <tr>
        <td><?php echo $new_record; ?></td>
        <td><?php echo date('F-d-Y', strtotime($row['date_created'])) ?></td>
        <td><?php echo $row['studnum']; ?></td>
        <td><?php echo $row['fname'] . ' ' . $row['lname']; ?></td>
        <td><?php echo $row['course']; ?></td>
        <td><?php echo (!empty($row['room']) && !empty($row['dorm_name']) && $row['room'] != ' ' && $row['dorm_name'] != ' ') ? $row['dorm_name'] . ' <b>' . $row['room'] . '</b>' : '<b style="color: Green;">Not assigned yet</b>'; ?></td>
        <td style="color: red;"><b><?php echo $row['status']; ?></b></td>
        <td>
            <a href="edit.php?id=<?php echo $row["tenants_id"]; ?>"><button class='btn bg-gradient-success'><i class="fa fa-edit"></i></button></a>
            <button id="<?php echo $row["tenants_id"]; ?>" class="eye-button btn bg-gradient-primary"><i class="fa fa-eye"></i></button>
            <button class='confirm_del_btn btn bg-gradient-danger' value="<?php echo $row['tenants_id']; ?>"><i class='fa fa-trash'></i></button>
...
?>
                ...
                <br><br><br><br><div class="modal-dialog modal-dialog-centered" style="max-width: 800px;">
                            <div class="modal-content animate">
                            <!-- Modal Header -->
                            <div class="modal-header">
                                <h4 class="modal-title">Student Boarder Information</h4>
